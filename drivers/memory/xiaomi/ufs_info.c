...
static ssize_t err_reason_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (g_ufs_info) {
		...
		ret = snprintf(buf, PAGE_SIZE, "%s%s%s%s%s%s%s%s%s%s",
									host->err_stats.err_reason,
									host->err_stats.err_reason+1,
									host->err_stats.err_reason+2,
									host->err_stats.err_reason+3,
									host->err_stats.err_reason+4,
									host->err_stats.err_reason+5,
									host->err_stats.err_reason+6,
									host->err_stats.err_reason+7,
									host->err_stats.err_reason+8,
									host->err_stats.err_reason+9);
	}
	return ret;
}
