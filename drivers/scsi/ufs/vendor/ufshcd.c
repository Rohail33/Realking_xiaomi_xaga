...
#endif
#ifdef CONFIG_SCSI_UFS_XIAOMI_CLD
#include "feature/cld/ufscld.h"
#endif
...
void ufsdbg_set_err_state(struct ufs_hba *hba, char *err_reason)
{
	...
	while (offset) {
		memcpy(host->err_stats.err_reason + offset, 
				host->err_stats.err_reason + offset - 1, 32);
		offset--;
	}
	memcpy(host->err_stats.err_reason, err_reason, 32);
	dump_stack();
}
...
int ufshcd_query_flag_sel_retry(struct ufs_hba *hba, enum query_opcode opcode,
...
int ufshcd_uic_hibern8_enter(struct ufs_hba *hba)
{
	...
	if (ret) {
		...
		ufsdbg_set_err_state(hba, "hibern8_enter_fail\n");
#endif
...
static int ufshcd_wb_toggle_flush_during_h8(struct ufs_hba *hba, bool set)
{
	...
	hba->wb_flush_during_h8 = set ? true:false;
