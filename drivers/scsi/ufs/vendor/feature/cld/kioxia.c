...
int kioxia_cld_set_trigger(struct ufscld_dev *cld, u32 trigger)
{
	...
	return ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG, (enum attr_idn)QUERY_ATTR_IDN_CLD_ENABLE, 0, 0, &trigger);
}
...
int kioxia_cld_get_trigger(struct ufscld_dev *cld, u32 *trigger)
{
	...
	return ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_READ_FLAG, (enum attr_idn)QUERY_ATTR_IDN_CLD_ENABLE, 0, 0, trigger);
}
