...
int micron_cld_set_trigger(struct ufscld_dev *cld, u32 trigger)
{
	struct ufs_hba *hba = cld->hba;
	if (trigger)
		return ufshcd_query_flag_sel_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG, (enum flag_idn)QUERY_FLAG_IDN_CLD_ENABLE, 0, 0, NULL);
	else
		return ufshcd_query_flag_sel_retry(hba, UPIU_QUERY_OPCODE_CLEAR_FLAG, (enum flag_idn)QUERY_FLAG_IDN_CLD_ENABLE, 0, 0, NULL);
}

int micron_cld_get_trigger(struct ufscld_dev *cld, u32 *trigger)
{
	struct ufs_hba *hba = cld->hba;
	return ufshcd_query_flag_sel_retry(hba, UPIU_QUERY_OPCODE_READ_FLAG, (enum flag_idn)QUERY_FLAG_IDN_CLD_ENABLE, 0, 0, (bool *)trigger);
}
...
int micron_cld_operation_status(struct ufscld_dev *cld, int *op_status)
{
	...
	if (defragprogress == DEFRAG_OPERATION_COMPLETED || defragprogress == DEFRAG_OPERATION_STOPPED) {// if cld was done
